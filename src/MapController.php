...
class MapController
{
    ...
    private static function documentUrl(int $documents_id, int $items_id): string
    {
        ...
        // Bloco 5i-2: o alvo deixou de ser o front/document.send.php do CORE
        // (cujo canViewFile cobra direitos nativos de Documentos - provado em
        // tela: miniatura quebrada no perfil sem Gerencia > Documentos) e
        // passou a ser o endpoint do PLUGIN, cujo porteiro e' o Ler do DGO+.
        // Ponto unico: faixa de miniaturas e lista do gerenciador montam o
        // link SO' por aqui - retarget aqui trocou o porteiro das duas telas.
        return ($CFG_GLPI['root_doc'] ?? '') . '/plugins/dgoplus/front/document.send.php?' . http_build_query([
            'docid'    => $documents_id,
            'items_id' => $items_id,
        ]);
    }
